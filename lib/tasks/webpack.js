...
const getWebpackConfigClient = require('../../config/webpack.config.client');
...
function runWebpack(options) {
  const config = getWebpackConfigClient(options);
...
function startWebpack(options) {
  ...
  config.output.publicPath = `http://localhost:${projectConfig.servers.cdn.port()}/`;
...
function getCompiler(config, {debug}) {
  const defaultEntryPath = path.join(config.context, projectConfig.defaultEntry());
  if (projectConfig.entry() || existsJs(defaultEntryPath)) {
